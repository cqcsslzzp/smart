...
public class UserDao {
    ...
    private final static String MATC_COUNT_SQL = "SELECT COUNT(*) FROM t_user WHERE user_name=? AND password=?";

    private final static String UPDATE_LOGIN_INFO_SQL = "UPDATE t_user SET last_visit=?,last_ip=?,credits=? WHERE user_id=?";

    @Autowired  //自动注入JDBTEPLATE的bean
    ...
    public User findUserByUserName(final String userName) {
        ...
        jdbcTemplate.query(MATC_COUNT_SQL, new Object[]{userName}, new RowCallbackHandler() {
            @Override
